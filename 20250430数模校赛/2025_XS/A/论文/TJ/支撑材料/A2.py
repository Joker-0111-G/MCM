...
def process_station(station_name, config):
    ...
    # 理论功率与DI
    df['理论功率'] = df.apply(
        lambda row: compute_theoretical_power(row, eta, alpha1, beta_h, tref, h0_fit, h1_fit, ifactor), axis=1)
    df['DI'] = (df['理论功率'] - df['实际功率']) / df['理论功率'].replace(0, np.nan) * 100
    df['DI'] = df['DI'].clip(lower=0)
    df['DI均值'], df['DI波动'] = sliding_statistics(df['DI'])

    # 预警判断
    theta1, theta2 = (3, 4) if station_name == '电站1' else (10, 5)
    df['预警'] = (df['DI均值'] > theta1) & (df['DI波动'] < theta2)

    # 保存结果
    df.reset_index().to_excel(f"A2/ANS/{station_name}_DI分析结果.xlsx", index=False)
    ...
    plt.savefig(f"A2/ANS/{station_name}_DI趋势图.png")
    plt.show()
    plt.close()
