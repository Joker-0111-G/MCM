...
def dynamic_cleaning_count(df, cap, price_list, p_electric=0.582, theta_safe=20, cooldown_hours=72):
    result = {}
    for price in price_list:
        ...
        df_copy = df.copy()
        df_copy['发电损失'] = df_copy['DI'] / 100 * df_copy['实际功率'] * p_electric
        df_copy['清洗建议'] = False

        for t in df_copy.index:
            # 冷却期判断 3天内不再清洗
            if last_clean_time is not None and (t - last_clean_time).total_seconds() < cooldown_hours * 3600:
                continue

            acc_loss += df_copy.at[t, '发电损失']
            di_mean = df_copy.at[t, 'DI均值']
            triggered = False

            if acc_loss >= C_clean:
                triggered = True
            elif di_mean > theta_safe:
                triggered = True

            if triggered:
                df_copy.at[t, '清洗建议'] = True
                cleaned_times += 1
                acc_loss = 0
                last_clean_time = t  # 设置当前清洗时间，启动冷却期

        result[round(price, 1)] = cleaned_times
    return result
...
def process_station(station_name, config):
    ...
    # 模拟不同价格下的清洗次数
    price_range = np.arange(0.1, 5.1, 0.1)  #
    result = dynamic_cleaning_count(df, cap, price_range)

    # 保存CSV
    os.makedirs("A3/ANS_2", exist_ok=True)
    pd.DataFrame(list(result.items()), columns=['清洗价格(元/kW)', '清洗次数']).to_excel(f"A3/ANS_2/{station_name}_价格清洗次数统计.xlsx", index=False)

    # 绘图
    plt.figure(figsize=(10, 5))
    plt.plot(list(result.keys()), list(result.values()), marker='o', label=station_name)
    plt.title(f"{station_name} 不同清洗价格下清洗次数变化趋势")
    plt.xlabel("清洗价格 (元/kW)")
    plt.ylabel("清洗次数")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"A3/ANS_2/{station_name}_清洗价格影响趋势图.png")
    plt.close()
