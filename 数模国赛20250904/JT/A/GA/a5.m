%% 三维空间多导弹-多无人机协同烟雾遮蔽模拟 (最终版遗传算法 - 新目标函数)
%
% ***********************************
% 描述:
...
%
% 新版目标函数 (2025-09-05):
% 最大化目标被【任意一枚或多枚导弹】遮蔽的总时长 (逻辑 OR)。
% 这样可以计算所有有效遮蔽时间的并集，自动去除重叠部分。
%
% 性能警告:
% 本脚本为单文件版，并行计算 'parfor' 已被替换为常规 'for' 循环。
% 这将导致计算时间显著增加。
% ***********************************
...
ylabel('最优适应度 (最大总遮蔽时长 s)');
grid on;

filename = 'a5.png'; % 定义图片文件名
...
function fitness = calculateFitness_MultiMulti(individual, all_missile_trajs, all_target_points, t_vec_full, t_end)
    ...
    for i = 1:length(t_vec_full)
        ...
        % *** 核心逻辑修改：从 AND 改为 OR，最大化任意遮蔽时间 ***
        % 只要目标被任意一枚导弹的视线遮蔽，该时刻就有效
...
function all_grenade_params = decodeIndividual(individual, t_end)
    ...
    for i = 1:5 % 遍历5架无人机
        ...
        % 弹 1
        % 物理模型：无人机从初始点出发，以(v_fy1, theta_yaw1)飞行t_deploy1秒后，到达投放点
        ...
        % 弹 2
        % 物理模型：无人机从弹1投放点出发，以(v_fy2, theta_yaw2)飞行(t_deploy2-t_deploy1)秒后，到达投放点
        ...
        % 弹 3
        % 物理模型：无人机从弹2投放点出发，以(v_fy3, theta_yaw3)飞行(t_deploy3-t_deploy2)秒后，到达投放点
